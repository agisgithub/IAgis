...
def test_admin_routing_and_bindings(tmp_path):
    repo=Repository(tmp_path/"admin.db")
    repo.add_model("Qwen local","ollama","qwen:test","http://127.0.0.1:11434",60)
    repo.add_agent("Homologação","Analisa pedidos","Faça perguntas",1)
    repo.add_skill("JSON","Normaliza",'{"type":"object"}','{"type":"object"}',
                   'def run(input_data):\n    return {}')
    repo.set_enabled("skills",1,True)
    repo.bind_skill(1,1)
    repo.add_rule("software",10,"titulo","software",1)
